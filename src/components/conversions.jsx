...
const Conversion = () => {
	...
	const getrateInfo = () => {
		...
		axios
			.get(rateInfoUrl) //using axios to get the data
			.then(({ data }) => {
				setrateInfo(data);
				setrArray([
					//putting the currency rates into an array
	...
	if (rateInfo !== null) {
		//checking when the object values are not null
		...
		return (
			<div>
				...
				{/* Getting time information using UTC */}
				<p className="b">The local time is {newDate}</p>{' '}
				{/* Printing local time information */}
				<p className="x"> ⌄</p>
				<button className="btn" onClick={orderConversion}>
					Sort Exchange Rates
				</button>
					...
					<div key={data.id}>
						<div>
							{data == rateInfo.bpi.USD.rate_float ? (
								<>
									<p className="e">1 USD is {conversionUSDc} BTC</p>
									<p className="e"> and 1 BTC is ${rateInfo.bpi.USD.rate_float} USD</p>
								</>
							) : data == rateInfo.bpi.EUR.rate_float ? (
								<>
									<p className="c">1 EUR is {conversionEURc} BTC</p>
									<p className="c"> and 1 BTC is {rateInfo.bpi.EUR.rate_float} EUR</p>
								</>
							) : (
								<>
									<p className="d">1 GBP is {conversionGBPc} BTC</p>
									<p className="d"> and 1 BTC is {rateInfo.bpi.GBP.rate_float} GBP</p>
								</>
							)}
						</div>
					</div>
				...
				<div className="m">
					...
					<input
						...
						value={num}
					></input>{' '}
					{/* input feild that takes in a number */}
				</div>
				<br></br>
				<button className="btn" onClick={getNewNum}>
					Submit
				</button>{' '}
				{/* button that produces conversion */}
				<p className="b"> The converted value is {conversionVal} BTC</p>
